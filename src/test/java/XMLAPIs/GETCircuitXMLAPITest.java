...
public class GETCircuitXMLAPITest {
	...
	//Note: 99% of the times we will be dealing with Json only in real time projects, XML might be asked in interview.
	@Test
	...
	public void xmlTest() {
		...
		Response response = RestAssured.given()
				...
				.get("/api/f1/2017/circuits.xml")
				.then()
				.extract().response(); //This is the same response as what we will get after we stop at .get() method as well. Just another way.
		...
		List<String> circuitNames = xmlPath.getList("MRData.CircuitTable.Circuit.CircuitName"); //For child we use dots(.)
		for(String e : circuitNames) {
			System.out.println(e);
		}
		...
		List<String> circuitIds = xmlPath.getList("MRData.CircuitTable.Circuit.@circuitId"); //For attribute we use @
		...
		//fetch the locality where cicruitId = americas
		String locality = xmlPath.get("**.findAll { it.@circuitId == 'americas' }.Location.Locality").toString(); //** means deep scan, 'it' is a groovy object already available in RestAssured.  
		System.out.println(locality);
